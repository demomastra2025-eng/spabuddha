## Buddha Spa Certificates

Фронтенд/бэкенд приложение для продажи подарочных сертификатов Buddha Spa:

- шаговый конструктор сертификата (филиал, данные получателя, дизайн, доставка, оплата);
- предпросмотр с генерацией PDF;
- интеграции OneVision (оплата), Resend/SMTP (email), Wazzup (WhatsApp);
- админ‑панель для управления филиалами, шаблонами и пользователями; список услуг и номиналов подтягивается из Altegio.

### Быстрый старт

```bash
pnpm install
cp .env.example .env  # заполните переменные окружения
pnpm dev              # запускает API (Express) и Vite
```

Приложение доступно на http://localhost:5173, API — на http://localhost:3000.

### Развертывание через Docker

Для запуска приложения и базы данных в Docker-контейнерах:

```bash
# 1. Создайте .env файл (или используйте готовый .env.docker как шаблон)
cp .env.docker .env

# 2. Отредактируйте .env и заполните необходимые переменные:
#    - NEXTAUTH_SECRET (сгенерировать можно через: openssl rand -base64 32)
#    - JWT_SECRET
#    - RESEND_API_KEY (для e-mail)
#    - И другие API ключи по необходимости

# 3. Запустите контейнеры
docker-compose up -d

# 4. Проверьте логи
docker-compose logs -f app

# 5. Остановка контейнеров
docker-compose down

# 6. Пересборка после изменения кода
docker-compose up -d --build
```

Приложение будет доступно на http://localhost:3000, PostgreSQL на порту 5432.

**Примечание**: База данных автоматически инициализируется из `schema.sql` при первом запуске.


### Основные переменные окружения

| Переменная | Назначение |
|-----------|-----------|
| `DATABASE_URL` | строка подключения к PostgreSQL |
| `APP_BASE_URL` | публичный адрес фронтенда (нужен для callback’ов) |
| `WAZZUP_API_URL` | базовый URL Wazzup API (токен/канал задаются у филиала в админке) |
| `RESEND_*` или SMTP‑параметры | отправка e-mail |
| `ONEVISION_*` | настройки платёжного провайдера |
| `ALTEGIO_GOODS_SYNC_CRON` | cron-выражение для фонового обновления кэша товаров Altegio (по умолчанию раз в час) |

Полный список см. в `src/server/config/env.ts`.

### Основные команды

| Команда | Описание |
|---------|----------|
| `pnpm dev` | локальный запуск API + клиент |
| `pnpm dev:ngrok` | dev-сервер + туннель Ngrok для тестов callback’ов |
| `pnpm build` | сборка клиента |
| `pnpm lint` | ESLint для всего репо |
| `pnpm db:schema` | применение `schema.sql` |
| `pnpm seed` | первичное заполнение БД (idempotent) |

### Структура

- `server.ts`, `src/server` — Express API.
- `src/components`, `src/pages` — React/Vite интерфейс.
- `src/components/create/*` — шаги мастера оформления сертификата.
- `src/pages/AdminDashboard.tsx` — административный интерфейс.

### Первичное наполнение БД

- Локально: `pnpm seed` (можно гонять перед каждым запуском — скрипт idempotent).
- Прод: разовый запуск `pnpm seed` после применения `schema.sql`/миграций. Дальше список услуг/номиналов приходит из Altegio по API, локальный справочник не используется.

### Тестовые интеграции

Для отправки тестовых сертификатов без оплаты доступны эндпоинты `/api/preview/email` и `/api/preview/whatsapp`, вызываемые кнопками в предпросмотре. Убедитесь, что настроены `RESEND_*`/SMTP и у выбранного филиала заполнены поля Wazzup.

### Фоновый опрос OneVision

При старте API запускается job (`startOneVisionStatusPolling`), который каждые 2 минуты выбирает незавершённые платежи OneVision и вызывает `payment/status`. Ответ обрабатывается теми же правилами, что и вебхук, поэтому даже при потере callback’а заказ автоматически переведётся в `paid`/`failed`.

### Кэш Altegio товаров

- Ежечасно (или по cron в `ALTEGIO_GOODS_SYNC_CRON`) подтягиваем товары Altegio по `good_ids` каждого филиала через endpoint `goods/{company_id}/{good_id}` и сохраняем в `altegio_goods_cache`.
- После изменения `good_ids` у филиала кэш пересчитывается сразу в фоне.
- Публичные и админские эндпоинты `/api/altegio/*/goods` теперь читают только из кэша и не ходят в Altegio на каждый запрос.

### Лицензия

Проект предназначен для внутреннего использования Buddha Spa. Распространение и форки по согласованию.
